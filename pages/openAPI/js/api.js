   /*-----------Name-Marvel-Form-------------------*/
   const marvelForm = document.getElementById('marvelForm');

   marvelForm.addEventListener('submit', function(event) {
   ...
   function createURL() {
     ...
     // Generate the hash required for API authentication (md5 of ts + privateKey + publicKey)
     const hash = md5(ts + privateKey + publicKey);
   
     // Create the query parameters using URLSearchParams
     ...
     return url; // Return the complete API request URL
   }
   
   // Generate the API URL
   const url = createURL();
   
   // Fetch data from the Marvel API using the generated URL
   fetch(url)
     .then(response => {
       if (!response.ok) {
